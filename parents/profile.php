...
if (hash_equals($_SESSION['token'], $_POST['csrftoken'])) {
 // Getting Parent id   
 $parentid=intval($_SESSION['prntid']);   
// Getting Post Values    
...
$sql="update tblparents set parentName=:pname,parentAddress=:paddress,parentRelStudent=:prelstd,parentContact=:pcontact,parentOccupation=:poccupation,parentJob=:pjob where id=:parentid";
...
<input type="hidden" name="csrftoken" value="<?php echo htmlentities($_SESSION['token']); ?>" />
  ...
  <?php 
$parentid=intval($_SESSION['prntid']);
$sql="SELECT  id,parentName,parentAddress,parentRelStudent,parentContact,parentOccupation,parentJob,parentUsername,regDate from tblparents where id=:pid ";
$query = $dbh -> prepare($sql);
...
<div class="col-md-6 mt-10">
<input class="form-control" type="text" value="<?php echo htmlentities($result->parentUsername)?>" name="username" id="username" readonly="true" title="Username can't be change" style="border: solid 1px red" >
</div>
</div>  
